...
db.all(`SELECT id, ticket_id, first_name, last_name, username, email, address, issue, status, created_at FROM tickets ORDER BY created_at DESC`, [], (err, rows) => {
  ...
  for (const r of rows) {
    lines.push([
      ...
      r.username, r.email, r.address, r.issue, r.status, r.created_at
    ].map(escape).join(','));
  }
